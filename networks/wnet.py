...
if __name__ == '__main__':
    ...
    model = WNet(in_c=3, n_classes=1, layers=[8,16,32], conv_bridge=True, shortcut=True)
    print("Total params: {0:,}".format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
    print('Forward pass (bs={:d}) when running in the cpu:'.format(batch_size))
    start_time = time.time()
    logits = model(batch)
    if isinstance(logits, tuple):
        print("Yeah code works well")
        logits_aux, logits = logits
        print(logits.size(), logits_aux.size())
